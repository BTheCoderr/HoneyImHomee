...
const MealCard = ({ meal, price, portionSize, addToCart }) => {
  ...
  return (
    <div className="meal-card">
      {meal.dealText && (
        <div className="meal-deal-badge">{meal.dealText}</div>
      )}
      ...
      <div className="meal-content">
        <div className="meal-header">
          ...
          <div className="meal-price-block">
            <span className="meal-price">${price || '12.00'}</span>
            {meal.dealText && (
              <span className="meal-deal"> / {meal.dealText}</span>
            )}
          </div>
        </div>
